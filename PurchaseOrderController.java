...
public class PurchaseOrderController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        PoProduct.setEntries(database.getSuggestionList("SELECT * FROM PURCHASE_ORDER;", "PRODUCT"));
        PoProduct.setEntries(database.getSuggestionList("SELECT * FROM SALE_ORDER;", "PRODUCT"));
        PoProduct.setPromptText("PRODUCT NAME");
        PoProductV.getChildren().add(PoProduct);

        PoStorage = new AutoCompleteTextField();
        PoStorage.setEntries(database.getSuggestionList("SELECT * FROM PURCHASE_ORDER;", "STORAGE"));
        ...
        PoRateMon.setOnAction((ActionEvent evt) -> {
            ...
            PoRateKg.setText(IntegerTextField.numformat(rateKg));
            PoPrice.setText(IntegerTextField.numformat(totalKg * rateKg));
        });
    ...
    private String generateOrder() {
        ...
        }else ID=String.valueOf(slID);
        
        if(slPoID<1000){
            Ssl=String.format("%03d",slPoID);
        }else Ssl=String.valueOf(slPoID);
        
        if(poID<1000){
            Tsl=String.format("%03d",poID);
        }else Tsl=String.valueOf(poID);
...
//        upOb.setIdPurchaseOrder(pKeyPurchaseOrder);
//        upOb.setOrderNo(ob.getOrderNo());
//        upOb.setOrderQuantity(ob.getQuantity());
//        upOb.setOrderUnit(ob.getUnit());
//        upOb.setOrderUnitType(ob.getUnitType());
//        upOb.setProduct(ob.getProduct());
//        upOb.setStorage(ob.getStorage());
//        upOb.setUpcomingQuantity(ob.getQuantity());
//        upOb.setUpcomingUnit(ob.getUnit());
//        upOb.setUpcomingUnitType(ob.getUnitType());
    ...
    private void PoSaveOrderButtClick(ActionEvent event) throws IOException {
        ...
        if (PoSaveOrderButt.getText() == "SAVE") {
