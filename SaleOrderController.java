...
public class SaleOrderController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        SoProduct.setEntries(database.getSuggestionList("SELECT * FROM PURCHASE_ORDER;", "PRODUCT"));
        SoProduct.setEntries(database.getSuggestionList("SELECT * FROM SALE_ORDER;", "PRODUCT"));
        ...
        SoRateMon.setOnAction((ActionEvent evt) -> {
            ...
            SoRateKg.setText(IntegerTextField.numformat(rateKg));
            SoPrice.setText(IntegerTextField.numformat(totalKg * rateKg));
        });
    ...
    private String generateOrder() {
        ...
        String query = "SELECT MAX(SERIAL) as orderno FROM SALE_ORDER";
        int soID = dontDeleteDB.getAIntDB(query, "orderno");

        soID++;
        saleOrderSerial = soID;
        String ID, Ssl, Tsl;
        
        if(clID<100){
            ID=String.format("%02d", clID);
        }else ID=String.valueOf(clID);
        
        if(clslID<1000){
            Ssl=String.format("%03d",clslID);
        }else Ssl=String.valueOf(clslID);
        
        if(soID<1000){
            Tsl=String.format("%03d",soID);
        }else Tsl=String.valueOf(soID);
    ...
    private void SaveOrderButtClick(ActionEvent event) throws IOException {
        ...
        if (customerSerial != 0) {
            ...
            dontDeleteDB.setADoubleDB(query, customerSerial,
                    SoCustomerName.getText(), SoInstitute.getText());
